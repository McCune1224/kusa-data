...
- `mix precommit` — compile (warnings-as-errors) + format + test. **Run this when done.**
- `mix setup` — deps.get + assets.setup + assets.build
- `source .env && mix phx.server` — start dev server (needs `ACCESS_TOKEN` in `.env`)
...
- Bearer token via `ACCESS_TOKEN` env var
- Client never raises — returns `{:ok, data}` or `{:error, reason}`
- HTTP 429 is retried with exponential backoff; GraphQL errors are returned as `{:error, {:graphql, errors}}`
- See https://developer.start.gg/reference/query.doc.html for schema reference
...
and 2) external `phx-hook` annotations where JavaScript object literals are defined and passed to the
